...
const Orders = () => {
  ...
  useEffect(() => {
    fetch(`https://rocky-reef-85960.herokuapp.com/orders`)
      .then((res) => res.json())
      .then((data) => {
        setOrders(data);
        setLoading(false);
  ...
  const handleStatusChange = (id, status) => {
    ...
    fetch("https://rocky-reef-85960.herokuapp.com/updateOrderStatus", {
      method: "put",
  ...
  const deletion = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://rocky-reef-85960.herokuapp.com/placeorder/${id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
